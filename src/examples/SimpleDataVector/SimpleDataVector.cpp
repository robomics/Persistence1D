/*! \file SimpleDataVector.cpp
    Basic example which demonstrates how to use the Persistence1D class for finding extrema
   features.
*/
/*! \example SimpleDataVector.cpp
   ...
   [persistence_base_dir]\\src\\examples\\SimpleDataVector\\SimpleDataVectorRes.ref with contents:
...
int main() {
  ...
  for (vector<TPairedExtrema>::iterator it = Extrema.begin(); it != Extrema.end(); it++) {
    cout << "Persistence: " << (*it).Persistence << " minimum index: " << (*it).MinIndex
