...
/*! \example FilesAndFilters.cpp
   ...
   [persistence_base_dir]\\src\\examples\\FilesAndFilters\\FilesAndFiltersRes.ref with contents:
...
int main() {
  ...
  // In this case, we choose a threshold between largest and smallest persistence, to select from of
  // the data.
  ...
  cout << "Global minimum value: " << globalMinValue << " index: " << globalMinIndex << endl
       << endl;
  ...
  // Now set a threshold greater than the largest persistence, such that no paired extrema are
  // returned:
  ...
  // Trying to print the results using the local pairs structure via PrintPairs will not print
  // anything
